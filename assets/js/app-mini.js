(function($){
    ...
    function switch_mode(){
        var modeJqObj = $(".switch-dark-mode");
        if($('body').hasClass('io-black-mode')){
            if(modeJqObj.attr("data-original-title"))
                modeJqObj.attr("data-original-title","日间模式");
            else
                modeJqObj.attr("title","日间模式");
            $(".mode-ico").removeClass("icon-night").addClass("icon-light");
        }
        else{
            if(modeJqObj.attr("data-original-title"))
                modeJqObj.attr("data-original-title","夜间模式");
            else
                modeJqObj.attr("title","夜间模式");
    ...
    function stickFooter() {
        ...
        if(mainFooter.hasClass('text-xs'))
        {
    ...
    $(document).on('click', '.ajax-list-home a', function(event) {
        event.preventDefault();
        loadAjax( $(this), $(this).parents('.ajax-list-home'), '.ajax-'+$(this).data('id') );

    });

    function loadAjax(t,parent,tabContentCssSelector){
        if( !t.hasClass('active') ){
            parent.find('a').removeClass('active');
            t.addClass('active');
            var tabContentJqObj = $(tabContentCssSelector);
            $('div.row.rc-' + t.data('pid')).each(function() {
                $(this).parent().removeClass('active');
            });
            if(tabContentJqObj.children(".url-card").length > 0){
                tabContentJqObj.parent().addClass('active')
                return;
            }
            ...
            if(tabContentJqObj.children(".ajax-loading").length == 0)
                tabContentJqObj.append('<div class="ajax-loading text-center rounded" style="position:absolute;display:flex;left:0;width:100%;top:-1rem;bottom:.5rem;background:rgba(125,125,125,.5)"><div class="col align-self-center"><i class="iconfont icon-loading icon-spin icon-2x"></i></div></div>');
            $.ajax({
                url: theme.ajaxurl,
                type: 'POST',
                dataType: 'html',
            ...
            .done(function(response) {
                if (response.trim()) {
                    tabContentJqObj.html('');
                    tabContentJqObj.append(response);
                    var url =  tabContentJqObj.children('#ajax-cat-url').data('url');
                    if(url)
